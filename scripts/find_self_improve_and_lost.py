...
FALLBACK_ROOTS = [
    ...
    "plugins",
    "realai",
    "_quarantine",  # lost/unique gold for self-improve + promote
...
def _walk_roots(roots: list[Path]) -> list[dict]:
    hits: list[dict] = []
    skip = {"node_modules", ".venv", "venv", "__pycache__", ".git", "dist", "build", ".next", "site-packages"}
    # Quarantine is huge — depth-cap + prefer keyword-heavy paths
    quar_max_files = int(os.environ.get("REALAI_QUAR_MAX_FILES") or "4000")
    for root in roots:
        if root.is_file():
            continue
        is_quar = root.name == "_quarantine" or "_quarantine" in root.parts
        n_quar = 0
        for p in root.rglob("*.py"):
            ...
            if is_quar:
                n_quar += 1
                if n_quar > quar_max_files:
                    break
                # skip deep recycle nests under quarantine
