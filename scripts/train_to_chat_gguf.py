...
def pause_vulkan() -> None:
    try:
        ...
        from vulkan_runtime import pause_vulkan as _pause

        _pause()
        return
    except Exception as e:
        log(f"pause via vulkan_runtime failed ({e}); falling back to taskkill")
...
def restart_chat(gguf: Optional[str] = None) -> Dict[str, Any]:
    """Prefer fast Vulkan-only resume; optionally also bounce full stack via ps1."""
    try:
        scripts_dir = Path(__file__).resolve().parent
        if str(scripts_dir) not in sys.path:
            sys.path.insert(0, str(scripts_dir))
        ...
        # After swap, prefer the chat_gguf that was just written
        result = resume_vulkan(gguf, force_restart=True, wait_s=180.0)
        log(f"vulkan resume: {result}")
        if result.get("ok"):
            return result
    except Exception as e:
        log(f"vulkan_runtime resume failed: {e}")

    ps1 = ROOT / "scripts" / "run_local_chat.ps1"
    shell = "pwsh" if shutil.which("pwsh") else "powershell"
    log("fallback: restarting full chat stack (Vulkan + orch)…")
    subprocess.Popen(
        [shell, "-ExecutionPolicy", "Bypass", "-File", str(ps1), "-SkipUI", "-SkipPromote"],
        cwd=str(ROOT),
    )
    return {"ok": True, "status": "stack_launched_async", "ps1": str(ps1)}
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    ap.add_argument(
        ...
        help="Auto-resume llama-server after train/swap (default ON)",
    )
    ap.add_argument(
        "--no-restart-chat",
        action="store_true",
        help="Leave Vulkan paused after train (VRAM stays free)",
    )
    ap.add_argument("--dry-run", action="store_true")
    ap.add_argument("--keep-f16", action="store_true", help="Keep intermediate f16 GGUF")
    args = ap.parse_args(argv)
    if args.no_restart_chat:
        args.restart_chat = False
    ...
    try:
        ...
        if args.restart_chat:
            # Prefer freshly swapped chat GGUF; else VRAM-safe 1.5B via vulkan_runtime
            target = str(chat_gguf) if chat_gguf.is_file() else None
            resume_info = restart_chat(target)
        ...
        if not args.restart_chat:
            log("Resume chat: python scripts\\vulkan_runtime.py resume")
    except Exception as e:
        result["ok"] = False
