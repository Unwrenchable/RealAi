...
- [ ] `pocket_target` near a pocket; object-leg = last `kind: "object"` (else `.to` near the pocket). **`pocket_unmakeable`** = one driven primary OB, outgoing ray + incoming ghost-side approach (walkthrough §3.5). Banks / combos / caroms: category extras. **Kick:** rail-before-OB cue leg is exempt from the ghost model; a direct **object** pot after contact still gets the outgoing-ray check only.
...
- [ ] **Jumped-over blocker ball** tagged `role: "blocker"` (infer only if unique within 2 of the hop; else `jump_blocker_ambiguous`) so `blocked_lane` exempts that obstacle only
