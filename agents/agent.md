...
## Deployment Help (New Strong Capability)

RealAI now has an enhanced **DevOps / Deployment Specialist** (see `agents/devops_agent.ts` + the new `agents/deployment-guide.md`).

It understands the full current ecosystem:
- **Goonforge (cookbook)** — the main launchpad + recently integrated FIZZ protocol (`/fizz`) + Supreme Goggles naming (`.fizz` domains etc.).
- **Atomic Fizz** — the separate full game repo (wasteland GPS, radio DJ voice, RealAi NPCs, complex secrets).
- **RealAI itself**.
- **Supreme Goggles / naming** (integrated).

Ask it things like:
- "setup local environment for goonforge"
- "prepare vercel deploy for cookbook frontend including the new goggles naming tab"
- "generate render.yaml + env checklist for atomic fizz backend on mainnet"
- "walk me through deploying the DomainRegistry contract for goggles then wiring it"
- "full local + vercel + contract deploy checklist for the current project"
- "troubleshoot this vercel build error" (paste error)

It will give project-aware, copy-paste ready steps, full config files (vercel.json, render.yaml, .env sections), contract commands (Hardhat + Anchor), and local setup wizards. It knows the monorepo gotchas, secret lists, post-launch naming flow, etc.

The `deployment-guide.md` file is the source of truth for this expertise — keep it updated as projects evolve. The DevOpsAgent has the runtime logic + templates.
