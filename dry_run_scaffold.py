...
Usage:
  python dry_run_scaffold.py --root /workspaces/RealAi --og /workspaces/RealAi/realai_og_mess/realai --out plan_phase4_preview
"""
...
# classification priorities (higher index = stronger match)
...
def build_preview(root, og_root, outdir):
    ...
    # classify files in root
    for rel, meta in files_root.items():
    ...
    # classify files in og
    for rel, meta in files_og.items():
        action, reason = simple_classify(rel)
        # OG copies are usually archive candidates unless identical to root
        if any(rel == pair[1] for pair in identical_pairs):
            action = "archive"
            reason = "OG duplicate (identical content)"
        else:
            # if OG contains core subsystems not present in root, mark merge
            if rel not in files_root:
                # if it looks like core subsystem, mark merge candidate
    ...
    # generate per-file diffs for items marked merge/rewrite
