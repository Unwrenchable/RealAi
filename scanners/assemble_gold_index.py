...
def from_realai_roots_ingest(bag: Dict[str, Dict[str, Any]], data: Any) -> None:
    """Folder-root candidates from scripts/realai_roots_*.json ingest."""
    if not data:
        return
    ...
    for e in queue:
        ...
        if not path:
            continue
        action = e.get("action") or "needs_review"
        if action not in ("promote", "needs_review", "rewrite", "archive_only", "skip_dup"):
            action = "needs_review"
        sub = e.get("subsystem") or subsystem_of(path)
        era = e.get("era") or era_of(path)
        reason = ",".join(e.get("reasons") or ["realai_roots_ingest"])
        key = e.get("id") or f"droot:{norm_path(path)}"
        add_candidate(
            bag,
            key=key,
            path=path,
            ...
            reason=reason,
            action=action,
            source="realai_roots_ingest",
            extra=e.get("extra") or {"name": e.get("name")},
        )
        if key in bag and e.get("target"):
            bag[key]["target"] = e["target"]
        if key in bag and isinstance(e.get("priority"), int):
            bag[key]["priority"] = max(bag[key]["priority"], int(e["priority"]))
...
def main() -> int:
    ...
    roots = load_json(SCAN / "realai_roots_ingest.json")
    ...
    from_realai_roots_ingest(bag, roots)
    ...
    meta = {
        ...
        "inputs": {
            ...
            "realai_roots_ingest": roots is not None,
        },
