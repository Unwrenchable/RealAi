...
Safety:
  ...
  - Memory snapshots â†’ recovered/ only (never live DB paths)
  - Training Downloads â†’ training/data/
  - agentx / registryClient â†’ confirm authority paths
"""
...
# basename â†’ preferred authority/staging target
CANONICAL_TARGETS = {
...
def plan_target(item: Dict[str, Any]) -> Tuple[Optional[str], str]:
    ...
    # needs_review non-memory â†’ recovered staging
    ...
    for item in queue:
        ...
        if dst.exists():
            ...
            if dh == h:
                ...
                continue
            # different content â€” do not overwrite authority without force
...
def write_report(result: Dict[str, Any]) -> None:
    ...
    for a in result["actions"]:
        if a.get("status") in ("copied", "would_copy", "already_identical"):
            lines.append(
                f"- `{a['status']}` Â· `{a.get('src','')}` â†’ `{a.get('dst','')}`"
            )
    lines.append("")
    ...
    if args.apply:
        ...
        if not train_readme.exists():
            train_readme.write_text(
                ...
                "- `realai_finetune_dataset.jsonl` â€” instruction/response samples\n"
                "- `agent_manifests_for_finetuning.json` â€” agent roles for finetune\n\n"
