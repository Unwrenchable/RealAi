...
- The static export is generated into `realai-frontend/out`.
...
3. Render will use `render.yaml` automatically, or configure manually:
   - **Root Directory**: `realai-frontend`
...
#### Render: frontend cannot reach the backend

**Problem:** The deployed frontend loads, but chat requests fail.

**Solution:**
- Ensure `NEXT_PUBLIC_REALAI_API_BASE` points to the Vercel backend origin.
- Do **not** include `/v1` in `NEXT_PUBLIC_REALAI_API_BASE`.
- Re-deploy the static site after changing Render environment variables.
