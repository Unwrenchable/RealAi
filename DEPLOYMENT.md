...
| `REALAI_VULKAN_FORWARD` | `auto` (Hive) / `off`. Proxy chat to Vulkan **only** when the base is loopback **and** this is not Render. Hosted Cloud API is an unconditional deny (`force` does not override). |
