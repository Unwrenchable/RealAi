"""
RealAI Main API Server - With Vulkan llama-server forwarding
"""
...
# Original imports
from realai import RealAI, PROVIDER_CONFIGS, PROVIDER_ENV_VARS, _KEY_PREFIX_TO_PROVIDER
    ...
    def do_POST(self):
        ...
        try:
            ...
            if parsed_path.path == '/v1/chat/completions':
                ...
                self._send_response(200, plan)

            elif parsed_path.path == '/v1/memory/store':
                mem = get_memory()
                mem['long_term'].remember(
                    body.get("experience", ""),
                    body.get("metadata")
                )
                self._send_response(200, {"status": "stored"})

            elif parsed_path.path == '/v1/memory/recall':
                mem = get_memory()
                memories = mem['long_term'].recall(
                    body.get("query", ""),
                    body.get("top_k", 5)
                )
                self._send_response(200, {"memories": memories})
