"""Voice interaction — one-shot ASR→chat→TTS via realai.voice; WS under apps/api."""
...
ABILITY = {
    ...
    "type": "ability",
    "status": "LIVE",
    "source": "realai.voice.provider + hive :8001 chat",
    "dest": "abilities/voice_streaming.py",
    "capabilities": ["voice", "asr", "tts", "streaming_ws", "hive"],
    "secrets_policy": "none",
}
    ...
    **kwargs: Any,
    ...
    if mode in {"status", "info", ""}:
        return {
            ...
            "voice_lab": "python -m realai.voice.lab_server  →  http://127.0.0.1:8890",
        }
    ...
    if not text_in and ctx.get("audio_b64"):
        ...
        try:
            audio = base64.b64decode(str(ctx.get("audio_b64") or ""))
        except Exception:
            audio = b""
        asr = provider.listen(audio)
        text_in = str(asr.get("text") or "").strip()
    ...
    tts = tts_run(
        ...
        context={
            ...
            "full_audio": bool(ctx.get("full_audio")),
        },
    )
    return {
        "ok": True,
