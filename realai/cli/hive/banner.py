...
def collect_status(ctx) -> Dict[str, Any]:
    ...
    try:
        ...
        if not out.get("hive"):
            ...
            # HTTP hive may set mode=None when the running orchestrator
            # could not import the bridge. setdefault will not replace None.
            if not out["hive"].get("hive_mode") and not out["hive"].get("mode"):
                out["hive"]["hive_mode"] = hs.get("hive_mode")
                out["hive"]["mode"] = hs.get("hive_mode")
                if hs.get("present") is not None:
                    out["hive"]["present"] = hs.get("present")
                if hs.get("missing") is not None:
                    out["hive"]["missing"] = hs.get("missing")
            else:
