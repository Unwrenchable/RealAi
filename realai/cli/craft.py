...
def tool_agents(query: str = "", limit: int = 30) -> dict[str, Any]:
    """Hive-first roster from .github/agents + agentx; orchestrator is fallback."""
    try:
        apply_workspace()
        q = (query or "").strip().lower()
        if q in ("--hive", "-hive"):
        ...
        # Local hive-first merge (overseer, coder, architect, analyst, memory, governor, router).
        lq = "" if q in ("", "reload", "test", "test --all", "--all") else q
        try:
            from realai.v3_runtime_bridge import list_agent_tools_agents
            local = list_agent_tools_agents(limit=max(1, int(limit)), query=lq)
            if isinstance(local, dict) and (local.get("agents") or lq == "hive"):
                local.setdefault("source", "v3_runtime_bridge:list_agent_tools_agents")
                local.setdefault("workspace", str(_ws()))
                local.setdefault("mode", "LOCAL")
                return local
        except Exception:
            pass
...
def plan_tools(user_text: str) -> list[tuple[str, dict[str, Any]]]:
    ...
    if re.search(r"\bwhere am i\b|\bwhat project\b|\bworkspace\b", low) and len(low) < 40:
        plans.append(("pwd", {}))
    if re.search(r"\blist agents\b|\bagents?\b", low) and len(low) < 80:
        aq = ""
        if re.search(r"--hive\b|\bhive\b", low):
            aq = "hive"
        else:
            m = re.match(r"^/?agents\s+(.+)$", low)
            if m:
                aq = m.group(1).strip()
        plans.append(("agents", {"query": aq} if aq else {}))
    ...
    for tr in tool_results:
        ...
        if name in ("improve", "heal"):
            ...
            lines.append(
                ...
                f"mode={r.get('mode')} ws={r.get('workspace')} root={r.get('root')}"
            )
            checks = r.get("checks") or []
            fails = [x for x in checks if isinstance(x, dict) and not x.get("ok")]
            for x in fails[:20]:
                lines.append(f"  - FAIL {x.get('name')}: {x.get('detail')}")
            if not fails:
                for x in checks[:12]:
                    if isinstance(x, dict):
                        lines.append(f"  - OK {x.get('name')}: {x.get('detail')}")
        elif name == "organs":
            ...
            if isinstance(raw, list):
                ag = raw
                count = r.get("count", len(ag))
            elif isinstance(raw, dict):
                inner = raw.get("agents")
                ag = inner if isinstance(inner, list) else []
                count = raw.get("count", len(ag))
            else:
                ag = []
                count = r.get("count", 0)
            hive = r.get("hive") if isinstance(r.get("hive"), dict) else {}
            extra = ""
            if hive:
                extra = f" hive_mode={hive.get('mode')} present={hive.get('present')} missing={hive.get('missing')}"
            lines.append(f"- **agents:** {count} shown{extra}")
            for a in ag[:12]:
                if isinstance(a, dict):
                    tag = " [hive]" if a.get("hive") else ""
                    lines.append(f"  - {a.get('id')}: {a.get('role')}{tag}")
                else:
                    lines.append(f"  - {a}")
        elif name == "multi":
...
def stream_reply(
    ...
    skip_llm = heavy_tools | status_tools
    if tool_results:
        names = [tr.get("tool") for tr in tool_results]
        if names and all(n in skip_llm for n in names):
...
class CraftSession:
    ...
    def handle_stream(self, user: str) -> str:
        ...
        try:
            ...
            print(
                f"[route: {_rt.get('target', '?')} via {_rt.get('backend', '?')} | {_rt.get('reason', '')}]",
                flush=True,
            )
            if str(_rt.get("privacy") or "") == "cloud-blocked":
        ...
        def run_script(script_name: str) -> dict[str, Any]:
            ...
            try:
                ...
                if checks and completed.returncode == 0:
                    found_any = False
                    art_info = []
                    search_roots = [home, ws]
                    try:
                        pr = _product()
                        if pr not in search_roots:
                            search_roots.append(pr)
                    except Exception:
                        pass
                    for rel in checks:
