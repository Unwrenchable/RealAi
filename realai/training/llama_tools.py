...
def _candidates(names: List[str], extra_dirs: Optional[List[Path]] = None) -> List[Path]:
    ...
    dirs.extend(
        [
            Path(r"C:\llama-vulkan"),
            Path(os.environ.get("REALAI_LLAMA_SERVER", r"C:\llama-vulkan\llama-server.exe")).expanduser().parent,
    ...
    if env_root:
        ...
        dirs.append(Path(env_root).expanduser() / "tools")
