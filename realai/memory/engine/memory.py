...
class RedisMemoryAdapter(MemoryAdapter):
    """Redis list-per-namespace KV. Falls back to sqlite if Redis is down."""

    def __init__(self, url: str | None = None, fallback_file: Path | None = None) -> None:
        import os

        self._url = url or os.environ.get("REDIS_URL") or os.environ.get("REALAI_REDIS_URL") or "redis://127.0.0.1:6379/0"
        self._prefix = os.environ.get("REALAI_REDIS_PREFIX") or "realai:mem:"
        self._client = None
        self._fallback: SQLiteMemoryAdapter | None = None
        fb = fallback_file or Path(os.environ.get("REALAI_HOME") or ".") / "memory" / "hive" / "redis_fallback.sqlite3"
        try:
            import redis  # type: ignore

            client = redis.Redis.from_url(self._url, decode_responses=True, socket_connect_timeout=0.4)
            client.ping()
            self._client = client
        except Exception:
            self._fallback = SQLiteMemoryAdapter(fb)
    ...
    def append(self, namespace: str, value: dict[str, Any]) -> None:
        if self._fallback is not None:
            self._fallback.append(namespace, value)
            return
        assert self._client is not None
        self._client.rpush(self._key(namespace), json.dumps(value))

    def read(self, namespace: str, limit: int = 20) -> list[dict[str, Any]]:
        if self._fallback is not None:
            return self._fallback.read(namespace, limit=limit)
        assert self._client is not None
        raw = self._client.lrange(self._key(namespace), -max(1, limit), -1)
    ...
    def search(self, namespace: str, query: str, k: int = 5) -> list[dict[str, Any]]:
        if self._fallback is not None:
            return self._fallback.search(namespace, query, k=k)
        lowered = query.lower()
        hits = [r for r in self.read(namespace, limit=200) if lowered in json.dumps(r).lower()]
        return hits[-k:]
