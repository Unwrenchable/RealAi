...
from realai.model_assets import repo_root
from realai.sdk.python.realai_client import RealAIClient
from realai.core.self_builder import SelfBuilder, select_builder_model as _select  # internal, we have a better picker below
...
def check_server(api_url: str) -> Dict[str, Any]:
    ...
    for m in data:
        ...
        if not mid:
            continue
        if m.get("weights_ready") is False:
            continue
        # Orchestrator lists OpenAI-style models (object=model) without type=chat
        if m.get("type") in (None, "chat") or m.get("object") == "model":
            if str(mid).startswith("realai-lora"):
                continue
            ready.append(mid)
    return {"health": health, "chat_models_ready": ready, "api_url": api_url}
...
def _best_local_coder(api_url: str) -> str:
    env = (os.environ.get("REALAI_DEFAULT_MODEL") or "").strip()
    order = (
    ...
    try:
        ...
        pass
    return env or "realai-default-coder"
...
def close_the_loop(
    *,
    ...
    if not report["phases"]["server"].get("chat_models_ready"):
        ...
        report["message"] = (
            "Start chat stack: realai-stack  (or python -m realai.v3_orchestrator). "
            "Legacy: python -m realai.server.app"
        )
        return report
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        "--api-url",
        default=os.environ.get("REALAI_API_URL")
