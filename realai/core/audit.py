...
def _state_path(name: str) -> str:
    """Persist audit/consent under REALAI_HOME (never under random cwd)."""
    home = (
        os.environ.get("REALAI_HOME")
        or os.environ.get("REALAI_ROOT")
        or str(Path(__file__).resolve().parents[1])
    )
    base = Path(home) / "logs" / "state"
    ...
    def __init__(
        ...
        Args:
            log_path: Path to the JSONL log file. Parent dirs are created
                automatically. Default: ``<REALAI_HOME>/logs/state/audit_log.jsonl``.
            max_bytes: Maximum file size in bytes before rotation (default 10 MB).
        """
        self._log_path = log_path or _state_path("audit_log.jsonl")
    ...
    def __init__(self, storage_path: str | None = None) -> None:
        ...
        Args:
            storage_path: Path to the JSON persistence file.
                ...
                Default: ``<REALAI_HOME>/logs/state/consent_store.json``.
        """
        self._storage_path = storage_path or _state_path("consent_store.json")
        self._lock = threading.Lock()
