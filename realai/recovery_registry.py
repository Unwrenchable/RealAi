...
# Canonical LoRA + GGUF root is C:\models\checkpoints_lora (env-overridable).
# Grok worktree / staged copies are fallbacks only.
def _lora_candidates() -> List[Path]:
    env_roots = []
    for key in ("REALAI_LORA_ROOT", "REALAI_MODELS_DIR", "REALAI_CHECKPOINTS_LORA"):
        raw = (os.environ.get(key) or "").strip()
        if not raw:
            continue
        p = Path(raw)
        # REALAI_MODELS_DIR may be C:\models or C:\models\checkpoints_lora
        if p.name.lower() != "checkpoints_lora" and (p / "checkpoints_lora").is_dir():
            env_roots.append(p / "checkpoints_lora")
        env_roots.append(p)
    return [
        *env_roots,
        Path(r"C:\models\checkpoints_lora"),
        Path(r"C:\models") / "checkpoints_lora",
        _ROOT / "models" / "checkpoints_lora",
...
LORA_CANDIDATES = _lora_candidates()
...
def inventory() -> Dict[str, Any]:
    ...
    return {
        ...
        "endpoints": {
            ...
            "tasks": "POST /v1/tasks ; GET /v1/tasks ; GET /v1/tasks/{id}",
            ...
            "mcp": "not present — do not invent /mcp (mcp_server.py still missing from live package)",
        },
    }
