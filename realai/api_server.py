...
_FUSION_UI_DIR = _REPO_ROOT / "fusion-ui"
...
class RealAIAPIHandler(BaseHTTPRequestHandler):
    ...
    def _send_bytes(self, status_code: int, body: bytes, content_type: str):
        """Send a raw byte response with CORS."""
        self.send_response(status_code)
    ...
    def _fusion_config_js(self) -> bytes:
        """Emit live Fusion UI config from the same settings the API uses."""
        api_key = settings.REALAI_API_KEY or ""
        payload = {
            "BACKEND_BASE": settings.api_base_url(),
            "API_KEY": api_key,
            "PORT": settings.PORT,
        }
        # When served by this process, prefer same-origin (empty BACKEND_BASE signal via path).
        # Keep absolute BACKEND_BASE for file:// / static hosts; browser script still prefers
        # same-origin under /fusion-ui/.
        return (
            "window.__REALAI_CONFIG__ = {0};\n".format(json.dumps(payload))
        ).encode("utf-8")

    def _serve_fusion_ui(self, request_path: str) -> bool:
        """Serve Fusion UI static files. Returns True if handled."""
        if request_path in ("/fusion-ui", "/fusion-ui/"):
            request_path = "/fusion-ui/index.html"
        ...
        target = (_FUSION_UI_DIR / rel).resolve()
        try:
            target.relative_to(_FUSION_UI_DIR.resolve())
        except ValueError:
            self._send_response(404, {"error": "Not found"})
            return True
        ...
        # Never serve env/secrets from the static tree.
    ...
    def do_GET(self):
        ...
        if parsed_path.path == '/health':
            ...
            self._send_response(200, {
                ...
                "model": settings.REALAI_MODEL,
            })

        elif self._serve_fusion_ui(parsed_path.path):
            return
...
def run_server(host: str = "0.0.0.0", port=None):
    """
    Start the RealAI API server.

    Args:
        host (str): Host to bind to
        port (int): Port to listen on (defaults to settings.PORT / env)
    """
    if port is None:
        port = settings.PORT
    ...
    print(f"API base (env-aligned): {settings.api_base_url()}")
    print(f"\n  *** Open the chat UI: http://{ui_host}:{port}/ ***")
    print(f"  *** Open Fusion UI:  http://{ui_host}:{port}/fusion-ui/ ***\n")
    print("Available endpoints:")
    print("  GET  /              Web chat UI (browser)")
    print("  GET  /ui            Web chat UI (browser, alias)")
    print("  GET  /fusion-ui/    Fusion Interface (static UI)")
