...
PROVIDERS = {
    ...
    "grok": {
        "label": "xAI / Grok (disabled for RealAI Bot chat)",
        "placeholder": "xai-... (optional; not used by RealAI Bot)",
        "help": "Not used by RealAI Bot. Chat is local-only.",
...
class RealAILauncher(tk.Tk):
    ...
    _BTN_STOP = "#c62828"
    _PAD = 14
    _API_BASE_URL = "http://127.0.0.1:8001"
    _DEFAULT_MODEL = "realai-2.0"
    _API_TIMEOUT = 30

    def __init__(self):
        super().__init__()
        self.title("RealAI Bot \u2014 Local Provider")
    ...
    def _build_ui(self) -> None:
        ...
        tk.Label(
            header,
            text="Local-first RealAI provider \u2014 chat uses local models (no Grok API)",
            font=("Segoe UI", 9),
            bg=self._HEADER_BG,
            fg=self._HEADER_SUB,
            pady=2,
    ...
    def _start_server(self) -> None:
        ...
        self._status_var.set("\U0001f680 API server started \u2014 http://127.0.0.1:8001")        
        # Enable chat UI
        self._chat_input.config(state="normal")
        self._send_btn.config(state="normal")
