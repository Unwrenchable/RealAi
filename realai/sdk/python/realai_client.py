...
def _default_api_url():
    """Resolve API base from the same env keys as the server / Fusion UI."""
    try:
        from realai.server_settings import default_api_base
        ...
        for key in ('REALAI_API_BASE', 'REALAI_API_URL', 'NEXT_PUBLIC_API_URL'):
            ...
            if value:
                return value.rstrip('/')
        return 'http://127.0.0.1:{0}'.format(os.environ.get('PORT', '8001'))
...
def create_client(api_url=None, timeout=30):
