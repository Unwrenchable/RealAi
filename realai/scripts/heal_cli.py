...
def main(argv: list[str]) -> int:
    ...
    if cmd == "promote":
        # APPLY by default. Use --dry-run for preview.
        dry = "--dry-run" in argv
        force = "--force" in argv
        from realai.self_heal import run_curated_promote
        r = run_curated_promote(apply=not dry, force=force)
        print("ok=", r.get("ok"), "apply=", not dry)
        if r.get("stdout_tail"):
            print(r["stdout_tail"][-1500:])
        return 0 if r.get("ok") else 1

    if cmd == "cycle":
        # APPLY curated by default
        dry = "--dry-run" in argv
        from realai.self_heal import run_full_cycle
        r = run_full_cycle(apply_promote=not dry)
