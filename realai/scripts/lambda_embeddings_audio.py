...
def create_transcription_response(body: Dict[str, Any]) -> Dict[str, Any]:
    """ASR — prefer core.voice Whisper; fall back to honest stub."""
    audio_file = body.get("file") or body.get("audio_file") or ""
    audio_bytes = body.get("audio_bytes") or b""
    ...
    try:
        ...
        asr = WhisperASR()
        result = asr.transcribe(audio_bytes if isinstance(audio_bytes, (bytes, bytearray)) else b"")
        text = str((result or {}).get("text") or "")
        live = getattr(asr, "_model", None) is not None and text and "unavailable" not in text.lower()
        return {
            "text": text
            or (
                f"[RealAI ASR fallback] file={audio_file or 'none'}; "
                "install faster-whisper for live transcription"
            ),
            "language": body.get("language") or "en",
            "realai": {
                "status": "live" if live else "partial_fallback",
                "backend": "whisper" if live else "whisper_fallback",
                "source": "core.voice.asr_whisper",
            },
        }
    except Exception as e:
        return {
            ...
            "realai": {
                ...
                "error": str(e),
                "next": "wire local whisper under core.voice",
            },
        }


def create_speech_response(body: Dict[str, Any]) -> Dict[str, Any]:
    """TTS — prefer core.voice Piper; fall back to honest stub."""
    ...
    try:
        ...
        wav = PiperTTS().synthesize(str(text))
        live = len(wav or b"") > 44
        return {
            "created": int(time.time()),
            "voice": body.get("voice") or "piper",
            "format": "wav",
            "text_echo": str(text)[:500],
            "audio_url": None,
            "audio_b64": base64.b64encode(wav or b"").decode("ascii") if wav else None,
            "bytes": len(wav or b""),
            "realai": {
                "status": "live" if live else "partial_fallback",
                "backend": "piper" if live else "piper_silent_fallback",
                "source": "core.voice.tts_piper",
            },
        }
    except Exception as e:
        return {
